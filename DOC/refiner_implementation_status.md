...
**现象**: 选中标注框后点击 zoom in，视图不会以选中框为中心放大，而是总是偏向右下角。

**文件**: `anno_refiner_app/src/ui/components.py`

**详细 Bug 报告**: `TEST_page_b_annotator/DOC/zoom_focus_bug.md`

**问题概述**:
- `InteractiveAnnotator` 组件使用 CSS `transform: translate() scale()` 实现缩放
- BBox 坐标在原始图像坐标系（如 1920x1080）
- 视口大小固定为 CSS 像素（如 900x600）
- 当图像大于视口时，坐标系统转换可能存在问题
- `set_zoom()` 和 `_get_zoom_focus_point()` 方法的 pan 计算可能没有正确处理图像与视口尺寸不一致的情况

**相关方法**:
- `set_zoom(zoom, focus_point)`
- `_get_zoom_focus_point()`
- `_apply_transform()`
- `_constrain_pan()`
